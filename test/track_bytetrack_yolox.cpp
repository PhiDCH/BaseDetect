...
vector<Object> convert_yoloxOutput_2_bytetrackInput(vector<YoloxOutput>& res) {
    vector<Object> tmp;
    tmp.resize(res.size());
...
int main (int argc, char** argv) {
    ...
    while (cap.read(img)) {
        ...
        auto res = convert_yoloxOutput_2_bytetrackInput(detector.result[0]);
        ...
        putText(img, format("yolox frame: %d fps: %d num: %d", frame_id, frame_id * 1000000 / total_ms, output_stracks.size()), Point(0, 60), 0, 2, Scalar(0, 0, 255), 6, LINE_AA);
        writer.write(img);
    }
